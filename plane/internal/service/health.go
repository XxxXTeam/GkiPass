...
type HealthService struct {
	...
	mu           sync.RWMutex
	logger       *zap.Logger
	stopChan     chan struct{} /* 用于停止后台 goroutine */
...
功能：先在读锁下收集过期节点 ID，释放后再加写锁处理，避免在 RLock 下写数据导致竞态。
*/
func (s *HealthService) checkNodesHealth() {
	...
	if len(expiredNodeIDs) > 0 {
		s.mu.Lock()
		for _, nodeID := range expiredNodeIDs {
...
// saveAlertToDB 保存告警到数据库（预留：待 HealthService 接入 DAO 后实现持久化）
func (s *HealthService) saveAlertToDB(alert AlertEvent) {
	s.logger.Debug("告警数据未持久化（HealthService 无 DAO）",
		zap.String("nodeID", alert.NodeID),
		zap.String("component", alert.Component))
}

// updateAlertInDB 更新数据库中的告警（预留：待 HealthService 接入 DAO 后实现持久化）
func (s *HealthService) updateAlertInDB(alert AlertEvent) {
	s.logger.Debug("告警状态未持久化（HealthService 无 DAO）",
		zap.String("nodeID", alert.NodeID),
		zap.Bool("cleared", alert.Cleared))
}
