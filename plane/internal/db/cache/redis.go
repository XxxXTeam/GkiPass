...
// SetSession 设置会话
func (r *RedisCache) SetSession(token string, session *Session, ttl time.Duration) error {
...
// GetSession 获取会话
func (r *RedisCache) GetSession(token string) (*Session, error) {
	key := fmt.Sprintf("session:%s", token)
...
func (r *RedisCache) SetNodeStatus(nodeID string, status *NodeStatus) error {
...
func (r *RedisCache) GetNodeStatus(nodeID string) (*NodeStatus, error) {
	key := fmt.Sprintf("node:status:%s", nodeID)
	data, err := r.client.Get(r.ctx, key).Bytes()
	if err == redis.Nil {
		return nil, nil
...
func (r *RedisCache) GetAllNodeStatus() (map[string]*NodeStatus, error) {
	pattern := "node:status:*"
