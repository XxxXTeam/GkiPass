...
func (h *NodeStatusHandler) GetNodeStatus(c *gin.Context) {
	...
	if len(node.Groups) > 0 {
		groupName = node.Groups[0].Name
	}

	isOnline := node.Status == "online" && time.Since(node.LastOnline) < 5*time.Minute

	// 计算运行时间
	uptime := int64(0)
	if isOnline {
		uptime = int64(time.Since(node.LastOnline).Seconds())
	}
...
func (h *NodeStatusHandler) ListNodesStatus(c *gin.Context) {
	...
	// 获取节点列表
	nodes, err := h.app.DAO.ListNodes(groupID, status, 1000, 0)
	if err != nil {
...
func (h *NodeStatusHandler) GetNodesByGroup(c *gin.Context) {
	...
	nodes, err := h.app.DAO.ListNodes(groupID, "", 1000, 0)
	if err != nil {
		response.InternalError(c, "Failed to list nodes")
		return
	}
