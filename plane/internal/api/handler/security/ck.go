...
func (h *CKHandler) GenerateNodeCK(c *gin.Context) {
	...
	// 生成 CK（30天有效期）
	ck, err := auth.CreateNodeCK(nodeID, 30*24*time.Hour)
	if err != nil {
		logger.Error("生成CK失败", zap.Error(err))
		response.InternalError(c, "Failed to generate connection key")
		return
	}

	// 保存到数据库
	if err := h.app.DAO.CreateConnectionKey(ck); err != nil {
...
func (h *CKHandler) ListNodeCKs(c *gin.Context) {
	...
	// 获取CK列表
	cks, err := h.app.DAO.GetConnectionKeysByNodeID(nodeID)
	if err != nil {
...
func (h *CKHandler) RevokeCK(c *gin.Context) {
	...
	if err := h.app.DAO.RevokeConnectionKey(ckID); err != nil {
